...
describe('AccountUsageCell', () => {
  ...
  it('OpenAI OAuth 点击行内刷新按钮时会重拉当前账号额度', async () => {
    getUsage
      ...
      .mockResolvedValueOnce({
        five_hour: {
          utilization: 22,
          resets_at: '2099-03-07T12:30:00Z',
        ...
        seven_day: {
          utilization: 40,
          resets_at: '2099-03-13T12:30:00Z',
          remaining_seconds: 1800,
          window_stats: {
            requests: 10,
            tokens: 1000,
    ...
    await wrapper.get('[data-testid="openai-usage-refresh"]').trigger('click')
    await flushPromises()

    expect(getUsage).toHaveBeenCalledTimes(2)
    expect(getUsage).toHaveBeenLastCalledWith(2011)
    expect(wrapper.text()).toContain('5h|22|1000')
  })
