  ...
  <div ref="rootRef" v-if="showUsageWindows">
    ...
    <template v-else-if="account.platform === 'openai' && account.type === 'oauth'">
      <div v-if="hasOpenAIUsageDisplay" class="space-y-1">
        <UsageProgressBar
          v-if="openAIUsageDisplay?.five_hour"
          label="5h"
          :utilization="openAIUsageDisplay.five_hour.utilization"
          :resets-at="openAIUsageDisplay.five_hour.resets_at"
          :window-stats="openAIUsageDisplay.five_hour.window_stats"
          :show-now-when-idle="true"
          color="indigo"
        />
        <UsageProgressBar
          v-if="openAIUsageDisplay?.seven_day"
          label="7d"
          :utilization="openAIUsageDisplay.seven_day.utilization"
          :resets-at="openAIUsageDisplay.seven_day.resets_at"
          :window-stats="openAIUsageDisplay.seven_day.window_stats"
          :show-now-when-idle="true"
          color="emerald"
        />
        <div class="mt-0.5 flex items-center gap-1.5">
          <button
            type="button"
            data-testid="openai-usage-refresh"
            class="inline-flex items-center gap-0.5 rounded px-1.5 py-0.5 text-[9px] font-medium text-blue-600 hover:bg-blue-50 dark:text-blue-400 dark:hover:bg-blue-900/30 transition-colors"
            :disabled="openAIRefreshLoading"
            @click="refreshOpenAIUsage"
          >
            <svg
              class="h-2.5 w-2.5"
              :class="{ 'animate-spin': openAIRefreshLoading }"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
...
const openAIRefreshLoading = ref(false)
...
const readNumericExtra = (key: string): number | null => {
...
const readTimeExtra = (key: string): string | null => {
  const extra = props.account.extra as Record<string, unknown> | undefined
  const value = extra?.[key]
  return typeof value === 'string' && value.trim() ? value : null
}

const buildOpenAIUsageFallbackProgress = (
  prefix: 'codex_5h' | 'codex_7d',
  legacyPercentKey: 'codex_primary_used_percent' | 'codex_secondary_used_percent',
  legacyResetAfterKey: 'codex_primary_reset_after_seconds' | 'codex_secondary_reset_after_seconds'
): UsageProgress | null => {
  const utilization = readNumericExtra(`${prefix}_used_percent`) ?? readNumericExtra(legacyPercentKey)
  const directResetsAt = readTimeExtra(`${prefix}_reset_at`)
  const resetAfterSeconds =
    readNumericExtra(`${prefix}_reset_after_seconds`) ?? readNumericExtra(legacyResetAfterKey)

  let resetsAt = directResetsAt
  if (!resetsAt && resetAfterSeconds != null) {
    const nextReset = new Date(Date.now() + resetAfterSeconds * 1000)
    resetsAt = nextReset.toISOString()
  }

  if (utilization == null && !resetsAt && resetAfterSeconds == null) {
    return null
  }

  const remainingSeconds = resetsAt
    ? Math.max(0, Math.floor((new Date(resetsAt).getTime() - Date.now()) / 1000))
    : Math.max(0, Math.floor(resetAfterSeconds ?? 0))
...
const openAIUsageFallback = computed<AccountUsageInfo | null>(() => {
  if (props.account.platform !== 'openai' || props.account.type !== 'oauth') return null

  const fiveHour = buildOpenAIUsageFallbackProgress(
    'codex_5h',
    'codex_primary_used_percent',
    'codex_primary_reset_after_seconds'
  )
  const sevenDay = buildOpenAIUsageFallbackProgress(
    'codex_7d',
  ...
  return {
    updated_at: readTimeExtra('codex_usage_updated_at'),
    five_hour: fiveHour,
    seven_day: sevenDay,
...
const openAIUsageDisplay = computed<AccountUsageInfo | null>(() => {
  if (props.account.platform !== 'openai' || props.account.type !== 'oauth') return null
  return usageInfo.value?.five_hour || usageInfo.value?.seven_day ? usageInfo.value : openAIUsageFallback.value
})

const hasOpenAIUsageDisplay = computed(() => {
  if (props.account.platform !== 'openai' || props.account.type !== 'oauth') return false
  return !!openAIUsageDisplay.value?.five_hour || !!openAIUsageDisplay.value?.seven_day
})
...
const refreshOpenAIUsage = async () => {
  ...
  openAIRefreshLoading.value = true
  try {
    const result = await adminAPI.accounts.getUsage(props.account.id)
    usageInfo.value = result
    _usageCache.set(props.account.id, { data: result, ts: Date.now() })
  } catch (e: any) {
    console.error('Failed to refresh OpenAI usage:', e)
  } finally {
    openAIRefreshLoading.value = false
  }
}
