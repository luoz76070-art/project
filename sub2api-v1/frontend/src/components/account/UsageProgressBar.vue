...
const barWidth = computed(() => {
  if (props.showNowWhenIdle) {
    return `${Math.max(0, Math.min(100 - props.utilization, 100))}%`
  }
...
const displayPercent = computed(() => {
  if (props.showNowWhenIdle) {
    const remaining = Math.max(0, 100 - props.utilization)
    return `余${Math.round(remaining)}%`
  }
