...
const (
	...
	// GeminiCLIOAuthClientID/Secret are intentionally not embedded in this repository.
	// If you want to keep using the Gemini CLI built-in OAuth flow, inject them via env.
...
func BuiltinGeminiCLIOAuthClientSecret() string {
	...
	return strings.TrimSpace(GeminiCLIOAuthClientSecret)
}

func IsBuiltinGeminiCLIOAuthClientID(clientID string) bool {
	builtin := BuiltinGeminiCLIOAuthClientID()
	return builtin != "" && strings.TrimSpace(clientID) == builtin
}
