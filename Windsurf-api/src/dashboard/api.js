...
import {
  listClientApiKeys, getClientApiKeySummary, createClientApiKey,
  updateClientApiKey, deleteClientApiKey,
} from '../client-api-keys.js';
...
export async function handleDashboardApi(method, subpath, body, req, res) {
  ...
  if (subpath === '/api-keys' && method === 'GET') {
    return json(res, 200, { summary: getClientApiKeySummary(), keys: listClientApiKeys() });
  }

  if (subpath === '/api-keys' && method === 'POST') {
    const key = createClientApiKey(body.label, body.note);
    return json(res, 200, { success: true, key, summary: getClientApiKeySummary() });
  }

  const clientKeyPatch = subpath.match(/^\/api-keys\/([^/]+)$/);
  if (clientKeyPatch && method === 'PATCH') {
    const key = updateClientApiKey(clientKeyPatch[1], body);
    if (!key) return json(res, 404, { error: 'API key not found' });
    return json(res, 200, { success: true, key, summary: getClientApiKeySummary() });
  }

  if (clientKeyPatch && method === 'DELETE') {
    const ok = deleteClientApiKey(clientKeyPatch[1]);
    return json(res, ok ? 200 : 404, { success: ok, summary: getClientApiKeySummary() });
  }
  ...
  if (subpath === '/overview' && method === 'GET') {
    ...
    return json(res, 200, {
      ...
      apiAuth: getClientApiKeySummary(),
