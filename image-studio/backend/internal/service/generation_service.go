...
func (s *GenerationService) runTask(ctx context.Context, t *model.GenerationTask) {
	...
	if t.Provider == model.ProviderGPT && t.Kind == string(provider.KindImage) && strings.EqualFold(t.ModelCode, "gpt-image-2") && shouldUseGPTWebRoute(params) && maxAttempts < 8 {
		maxAttempts = 8
	}
	...
	for attempt := 1; attempt <= maxAttempts; attempt++ {
		...
		if s.aes != nil {
			cred, derr := s.providerCredentialForRequest(ctx, acc, provReq)
			if derr != nil {
				lastErr = derr
...
func (s *GenerationService) providerCredentialForRequest(ctx context.Context, acc *model.Account, req *provider.Request) (string, error) {
	if shouldUseGPTImageWebCredential(req) {
		return s.gptOAuthAccessTokenForWeb(ctx, acc, req.ProxyURL)
	}
	proxyURL := ""
	if req != nil {
		proxyURL = req.ProxyURL
	}
	return s.providerCredential(ctx, acc, proxyURL)
}
...
func (s *GenerationService) pickAccountForTask(ctx context.Context, t *model.GenerationTask, params map[string]any) (*model.Account, error) {
	...
	if shouldUseGPTWebRoute(params) {
		if acc, err := s.pool.ReserveWhere(ctx, t.Provider, "round_robin", isGPTImageFreeWebAccount); err == nil {
			return acc, nil
		}
		if acc, err := s.pool.ReserveWhere(ctx, t.Provider, "round_robin", isGPTImageWebAccount); err == nil {
			return acc, nil
		}
	}
	if acc, err := s.pool.ReserveWhere(ctx, t.Provider, "round_robin", isGPTImageGatewayAccount); err == nil {
		return acc, nil
	}
...
func isGPTImageGatewayAccount(acc *model.Account) bool {
	if acc == nil || acc.Provider != model.ProviderGPT || acc.AuthType != model.AuthTypeAPIKey {
		return false
	}
	base := ""
	if acc.BaseURL != nil {
...
func isGPTImageFreeWebAccount(acc *model.Account) bool {
	return isGPTImageWebAccount(acc) && strings.EqualFold(accountPlanType(acc), "free")
}

func isGPTImageWebAccount(acc *model.Account) bool {
	if acc == nil || acc.Provider != model.ProviderGPT || acc.AuthType != model.AuthTypeOAuth {
		return false
	}
	base := ""
	if acc.BaseURL != nil {
...
func accountPlanType(acc *model.Account) string {
	...
	for _, key := range []string{"plan_type", "chatgpt_plan_type", "account_plan_type"} {
...
func shouldUseGPTImageWebCredential(req *provider.Request) bool {
	if req == nil || req.Account == nil || req.Account.Provider != model.ProviderGPT || req.Account.AuthType != model.AuthTypeOAuth {
		return false
	}
	if req.Kind != provider.KindImage || !strings.EqualFold(req.ModelCode, "gpt-image-2") || !shouldUseGPTWebRoute(req.Params) {
		return false
	}
...
func (s *GenerationService) gptOAuthAccessTokenForWeb(ctx context.Context, acc *model.Account, proxyURL string) (string, error) {
	at, err := s.decryptOptional(acc.AccessTokenEnc)
	if err != nil {
		return "", fmt.Errorf("decrypt access_token failed: %w", err)
	}
	if at != "" && !s.accessTokenExpiresWithin(acc, at, 5*time.Minute) {
		return at, nil
	}
	return s.gptOAuthAccessToken(ctx, acc, proxyURL)
}
...
func (s *GenerationService) accessTokenExpiresWithin(acc *model.Account, at string, window time.Duration) bool {
	if strings.TrimSpace(at) == "" {
		return true
	}
	expAt := acc.AccessTokenExpiresAt
	...
	return expAt.Before(time.Now().UTC().Add(window))
}
...
func retryableProviderError(err error) bool {
	...
	return isFatalOAuthRefreshError(err) ||
		...
		isGatewayPoolUnavailableError(msg) ||
		isGPTImageWebRetryableError(msg) ||
		isOAuthRefreshRetryableError(msg) ||
...
func isGatewayPoolUnavailableError(msg string) bool {
	if msg == "" {
		return false
	}
	return strings.Contains(msg, "号池暂无账号") ||
		strings.Contains(msg, "暂无可用账号") ||
		strings.Contains(msg, "未开放 image_generation") ||
		strings.Contains(msg, "不能用该号池原生生成") ||
		strings.Contains(msg, "tool choice 'image_generation' not found") ||
		strings.Contains(msg, "no available account")
}

func isGPTImageWebRetryableError(msg string) bool {
	if msg == "" || !strings.Contains(msg, "gpt image2 web") {
		return false
	}
	return strings.Contains(msg, "requires arkose") ||
		strings.Contains(msg, "requirements 401") ||
		strings.Contains(msg, "requirements 403") ||
		strings.Contains(msg, "requirements 429") ||
		strings.Contains(msg, "prepare 401") ||
		strings.Contains(msg, "prepare 403") ||
		strings.Contains(msg, "prepare 429") ||
		strings.Contains(msg, "conversation 401") ||
		strings.Contains(msg, "conversation 403") ||
		strings.Contains(msg, "conversation 429") ||
		strings.Contains(msg, "returned 0 image") ||
		strings.Contains(msg, "produced text instead of image")
}

func isOAuthRefreshRetryableError(msg string) bool {
	if !strings.Contains(msg, "refresh oauth access_token failed") {
		return false
	}
